...
No exceptions for "just debugging."
...
# GOOD
raise ValueError("Auth failed. Check your API key in config/.env.")
```
...
should be called at the boundary where the secret is actually used.

## Rules for `.gitignore`

The repo's [`.gitignore`](../../.gitignore) is the enforcement point. If you
introduce a new secret-bearing file location, **add it to `.gitignore` before
creating the file**.
...
1. Scan the diff for strings that look like tokens (length > 20, mix of
   alphanumerics, common prefixes like `sk-`, `gsk_`, `ghp_`).
