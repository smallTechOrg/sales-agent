...
export default function CampaignLeadPipelinePage({
  ...
  // When active run finishes, clear it and refresh the list.
  useEffect(() => {
    if (
      activeRun &&
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {activeRun && (activeRun.status === "pending" || activeRun.status === "running") && (
        <div className="mb-4 flex items-center gap-3 rounded-md border border-blue-500/30 bg-blue-500/10 px-4 py-3 text-sm text-blue-300">
          <Spinner />
          <span>
            Run <span className="font-mono text-xs">{activeRun.id.slice(0, 8)}</span> —{" "}
            <RunStatusBadge status={activeRun.status} />
            {activeRun.current_node && (
              <span className="ml-2 text-slate-400">node: <span className="font-mono">{activeRun.current_node}</span></span>
            )}
          </span>
        </div>
      )}
      ...
      <div className="flex gap-1 mb-6 border-b border-slate-800">
        {(["pipeline", "links", "runs"] as Tab[]).map((t) => (
          ...
          >
            {t === "pipeline" ? "Lead Pipeline" : t === "links" ? `Links (${links.length})` : `Runs (${runs.length})`}
          </button>
        ))}
      </div>
                ...
                <tbody className="divide-y divide-slate-800">
                  {runs.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="px-4 py-3 font-mono text-xs text-slate-400">{r.current_node ?? "—"}</td>
                      <td className="px-4 py-3 text-slate-300">{fmtDuration(r.started_at, r.finished_at)}</td>
